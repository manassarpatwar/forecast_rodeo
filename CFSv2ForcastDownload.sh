...
output="/data/aca18mms/forecast_rodeo/forecasts/cfsv2_2011-2018/"
curl -H "X-Dataverse-key: $token" https://dataverse.harvard.edu/api/datasets/:persistentId/versions/2/files?persistentId=doi:10.7910/DVN/CEFZLV | tee >(./jq '.data[].dataFile.persistentId' >persistentid) >(./jq '.data[].dataFile.filename' >filenames)
